...
import { clients, siteConfig, getWhatsAppUrl, getUniqueClientIndustries, getIndustryByName } from '@/data/site'
...
export default function PortfolioPage() {
  return (
    <>
      ...
      <section className="py-8 bg-white border-b border-border sticky top-20 z-40">
        <div className="container px-6">
          <div className="flex flex-wrap gap-3 justify-center">
            {getUniqueClientIndustries().map((industry) => {
              const industrySlug = industry.toLowerCase().replace(/\s+/g, '-').replace(/&/g, 'and')
              return (
                <a
                  key={industry}
                  href={`#${industrySlug}`}
                  className="px-4 py-2 text-sm font-medium text-text-secondary hover:text-accent hover:bg-accent/10 rounded-full transition-colors"
                >
                  {industry}
                </a>
              )
      ...
      {getUniqueClientIndustries().map((industry) => {
        const industrySlug = industry.toLowerCase().replace(/\s+/g, '-').replace(/&/g, 'and')
        const industryClients = Object.entries(clients).filter(([, client]) => client.industry === industry)
        const industryInfo = getIndustryByName(industry)

        if (industryClients.length === 0) return null

        return (
          <section key={industry} id={industrySlug} className="py-16 scroll-mt-32">
            <div className="container px-6">
              {/* Industry Header */}
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-10">
                <div>
                  <h2 className="text-2xl font-bold text-primary">{industry} Case Studies</h2>
                  <p className="text-text-secondary mt-1">
                    See how we help {industry.toLowerCase()} businesses grow with SEO and web design
                  </p>
                </div>
                {industryInfo && (
                  <Link
                    href={industryInfo.localSeoUrl}
                    className="inline-flex items-center gap-2 px-5 py-2.5 bg-accent/10 text-accent font-medium rounded-lg hover:bg-accent hover:text-white transition-colors"
                  >
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                {industryClients.map(([key, client]) => (
                  <div key={key} className="bg-white rounded-2xl border border-border overflow-hidden hover:shadow-xl transition-shadow group">
                    ...
                    <div className="p-6">
                      ...
                      <p className="text-sm text-text-secondary mb-4 line-clamp-3">{client.description}</p>
