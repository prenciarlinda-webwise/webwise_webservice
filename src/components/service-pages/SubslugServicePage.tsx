...
export default async function SubservicePage({ params }: { params: Promise<{ slug: string; subslug: string }> }) {
  ...
  const parentServiceUrl = slug === 'seo' ? `${siteConfig.url}/local-seo`
    : slug === 'web-development' ? `${siteConfig.url}/custom-web-development`
    : slug === 'digital-marketing' ? `${siteConfig.url}/digital-marketing`
    : `${siteConfig.url}/services/${slug}`

  const parentServicePath = slug === 'seo' ? '/local-seo'
      ...
      <section className="bg-gradient-to-br from-primary to-primary-dark py-20">
        <div className="container px-6">
          <nav className="flex items-center gap-2 text-white/60 text-sm mb-8 flex-wrap">
            ...
            <span>/</span>
            <Link href={parentServicePath} className="hover:text-white">{service.title}</Link>
            <span>/</span>
        ...
        <section className="py-24">
          <div className="container px-6">
            <div className="max-w-4xl mx-auto">
              ...
              <p className="text-lg text-text-secondary mb-6 leading-relaxed">{content.definition.answer}</p>
                ...
                {content.process.steps.map((step, i) => (
                  <div key={i} className="bg-white rounded-2xl p-8 border border-border shadow-sm">
                    <div className="flex items-start gap-6">
                      <div className="w-12 h-12 flex items-center justify-center bg-accent text-white font-bold rounded-xl flex-shrink-0">{i + 1}</div>
          ...
          <div className="container px-6">
            ...
            <div className="grid lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
              {content.packages.tiers.map((tier, i) => (
                <div key={i} className={`bg-white rounded-2xl border-2 p-8 flex flex-col relative ${i === 1 ? 'border-accent shadow-xl lg:scale-105' : 'border-border'}`}>
                  {i === 1 && (
                    <div className="absolute -top-4 left-1/2 -translate-x-1/2 px-4 py-1 bg-accent text-white text-sm font-medium rounded-full">Most Popular</div>
                  )}
                  <div className="mb-6">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
              {content.industries.list.map((industry, i) => (
                <div key={i} className="bg-white border border-border rounded-xl p-6 hover:shadow-lg transition-shadow">
                  ...
                  {industry.link && (
                    <Link href={industry.link} className="text-sm text-accent font-medium hover:underline">Read our guide →</Link>
                  )}
                </div>
              ))}
            </div>
              ...
              {content.caseStudies.studies.map((study, i) => (
                <div key={i} className="bg-white rounded-2xl p-8 border border-border">
                  <div className="flex items-start justify-between mb-4">
                    ...
                    {study.link && (
                      <Link href={study.link} className="text-sm text-accent font-medium hover:underline">Full case study →</Link>
                    )}
                  </div>
                  ...
                  <div className="bg-bg-secondary rounded-xl p-4">
                    ...
                    <p className="text-sm text-text-muted mt-3 text-center">Timeline: {study.timeline}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <section className="py-24">
        <div className="container px-6">
          <div className="bg-gradient-to-br from-primary to-primary-dark rounded-3xl p-12 lg:p-16 text-center text-white">
            ...
            <PricingCTA
              source={`${slug}/${subslug} — Footer CTA`}
